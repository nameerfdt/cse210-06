...
class ControlActorsAction(Action):
    ...
    def execute(self, cast, script):
        ...
        """
        # left
        if self._keyboard_service.is_key_down(pyray.KEY_LEFT):
            self._direction = Point(-constants.CELL_SIZE, 0)
        
        # right
        if self._keyboard_service.is_key_down(pyray.KEY_RIGHT):
            self._direction = Point(constants.CELL_SIZE, 0)       

        # up
        if self._keyboard_service.is_key_down(pyray.KEY_SPACE):
            self._direction = Point(constants.CELL_SIZE, 0) 
